...
7. **Run the approved request once.** After approval, call `run_design_research` with that approval ID. The tool atomically claims the approval before the OpenAI Responses API web search runs, with enforced domain filters and low search context. It records `consumed` on success or `failed` on an error; neither state can be reused for another paid search.
